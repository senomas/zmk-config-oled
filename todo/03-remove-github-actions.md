...
- [x] Add keymap-drawer target to Makefile
  ...
  - ⚙️ [x] Rewrote the keymap-drawer target to use pipx instead of Docker, as the reviewer flagged the previous Docker-based implementation as a gap. Changes:
    ...
    - 📋 Verified: the keymap-drawer target now uses `pipx run keymap-drawer` (no Docker), matching all spec requirements. All four keymaps processed via glob, `-c 12` used consistently, pipx-not-found and failure corner cases handled. Target placed in Primary targets section with .PHONY declaration and help entry.
...
- [x] Update README.md to reference local keymap-drawer
  ...
  - ⚙️ [x] Updated README.md to replace GitHub-Actions-centric keymap-drawer documentation with local Makefile-based instructions:
