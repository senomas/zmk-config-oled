...
- [x] Delete .github/workflows/build.yml and keymap-drawer.yaml `(15397738)`
      Remove both GitHub Actions workflow files from .github/workflows/. Clean removal — no other files reference them.
  - ⚙️ [x] Removed both workflow files. `(0ff0dbda)`
    - 📋 Verified: .github/workflows/ directory no longer exists — both build.yml and keymap-drawer.yaml have been deleted. No remaining references to either workflow file anywhere in the repo (checked README, Makefile, config/). `(054e1edc)`
- [x] Add keymap-drawer target to Makefile `(39495263)`
  ...
  - ⚙️ [x] Rewrote the keymap-drawer target to use pipx instead of Docker, as the reviewer flagged the previous Docker-based implementation as a gap. Changes: `(4199f92a)`
    
    1. Replaced Docker/Podman detection with pipx detection. If pipx is not found, prints: "pipx not found. Install with: python3 -m pip install --user pipx" and exits non-zero.
    2. Uses `pipx run keymap-drawer` as the CLI entry point (package: keymap-drawer, command: keymap).
    3. For each keymap in config/*.keymap (corne, lily58, sofle, splitkb_aurora_sofle):
       - `pipx run keymap-drawer parse -c config/config_keymap-drawer.yaml -c 12 -z <keymap> > keymap-drawer/<name>.yaml`
       - `pipx run keymap-drawer draw -c config/config_keymap-drawer.yaml keymap-drawer/<name>.yaml > keymap-drawer/<name>.svg`
    4. Uses -c 12 for all keymaps (they share the same physical corne-style layout).
    5. Any failure in pipx/keymap-drawer exits non-zero via `|| exit 1`.
    - 📋 ⚠️ GAP: The implementation uses Docker/Podman to run keymap-drawer inside a container (`zmkfirmware/zmk-build-arm:stable`) instead of `pipx run keymap-drawer` as specified. Key differences: `(e367552c)`
    ...
    - 📋 Verified: the keymap-drawer target now uses `pipx run keymap-drawer` (no Docker), matching all spec requirements. All four keymaps processed via glob, `-c 12` used consistently, pipx-not-found and failure corner cases handled. Target placed in Primary targets section with .PHONY declaration and help entry. `(51d6ec4f)`
- [x] Make build depend on keymap-drawer `(6ce6d23a)`
  ...
  - ⚙️ [x] Changed build prerequisite from codebase to codebase keymap-drawer. `(2382654a)`
    - 📋 Verified: the `build` target line now reads `build: codebase keymap-drawer` — exactly as specified. `keymap-drawer` runs first (it's lightweight, host-side) before the heavy Docker-based firmware build in `codebase`. The change is clean and correct. `(f585fcbe)`
- [x] Update README.md to reference local keymap-drawer `(39b68ead)`
  ...
  - ⚙️ [x] Updated README.md to replace GitHub-Actions-centric keymap-drawer documentation with local Makefile-based instructions: `(c88d08a3)`
    
    1. **INTRO section note (line 59):** Updated the SVG attribution note to also mention that SVGs are generated automatically with `make build`: "SVGs generated locally via `make keymap-drawer` (also runs automatically with `make build`)"
    
    2. **QUICK START section:** Added a new `[!NOTE]` block after the normal procedure explaining that keymap SVG images are generated locally via `make keymap-drawer` and automatically with `make build`, pointing to the `keymap-drawer/` folder for viewing them.
    
    3. **No references to `.github/workflows/keymap-drawer.yaml` found in README** — already clean from previous implementation.
    
    4. **LOCAL INSTALLATION tree view:** Already correctly shows "generated by `make keymap-drawer`" — no changes needed.
    - 📋 ⚠️ GAP: Three issues with the README update: `(46310384)`
    ...
    - 📋 Verified: all four spec items met. QUICK START [!NOTE] explains local generation via `make keymap-drawer`, mentions `make build` auto-triggers it, and points to keymap-drawer/ folder. No .github/workflows/ references remain. SVG image links intact. LOCAL INSTALLATION tree correctly shows keymap-drawer as generated by `make keymap-drawer`. `(e14fecee)`
